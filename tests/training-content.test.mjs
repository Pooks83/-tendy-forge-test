...
const validActivity=()=>({
 activityId:'two-hand-catch',version:1,familyId:'wall-ball-catch',name:'Wall ball two-hand catch',developmentPurpose:'Track a soft ball into controlled hands.',technicalSkillIds:[10,11],attributeIds:['SEE'],developmentAttributeIds:['TRACKING','HANDS'],
...
test('published activity requires both explicit reviews and publication time',()=>{
 ...
 assert.equal(content.isPublishedActivity({...value,developmentAttributeIds:[]}),false);
 assert.equal(content.isPublishedActivity({...value,developmentAttributeIds:['REFLEXES']}),false);
...
test('activity validation enforces exact instruction safety and prescription fields',()=>{
 ...
 assert.throws(()=>content.validateActivityVersion({...validActivity(),developmentAttributeIds:['TRACKING','TRACKING']}),/development attribute/i);
});
