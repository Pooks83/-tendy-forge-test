...
export async function GET(){
 try{return reply(await getActivePlayer(trainingDb(),await requireAdultIdentity()));}
 catch(error){
  ...
  console.error('Player request failed',error instanceof Error?error.message:'error');
  return reply({error:{code:'INTERNAL_RETRYABLE',message:'Your goalie could not be loaded. Try again.'}},503);
 }
}
