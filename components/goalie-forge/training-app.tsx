...
import {CompletionProgress,ProgressionPanel,type CompletionSummary,type ProgressionProjection} from '@/components/tendie-forge/progression-panel';
...
type Drill=ReturnType<typeof buildSession>['blocks'][number]&{activityId?:string;version?:number;familyId?:string;substitution?:Substitution|null};
...
type MissionProjection={id?:string;missionId:string;profileContextId:string;status:string;revision:number;currentActivityIndex:number;activities:MissionActivity[];executionSnapshot:MissionSnapshot|null;completionSummary?:CompletionSummary;pendingSync?:boolean;unavailable?:{code:string;message:string;nextAction:string};safetyHold?:{code:string;message:string;nextAction:string;activityKey:string;replacement:{activityId:string;version:number;name:string}|null}};
...
async function fetchProgressProjection():Promise<ProgressionProjection>{
 const response=await fetch('/api/progress',{cache:'no-store'});
 return await readTrainingResponse(response) as ProgressionProjection;
}
...
export function AdultSafetyReplacement({hold,busy,onAdopt}:{hold:NonNullable<MissionProjection['safetyHold']>;busy:boolean;onAdopt:()=>void}){
 return <section className="tf-error" role="alert"><AlertTriangle/><h2>Safety replacement required</h2><p>{hold.message}</p>{hold.replacement?<><p><strong>Reviewed replacement:</strong> {hold.replacement.name}</p><p>The withdrawn activity will restart. Any progress already recorded remains in the mission audit history.</p><button className="tf-secondary" disabled={busy} onClick={onAdopt}>{busy?'Saving…':'Use reviewed replacement'}</button></>:<p><strong>No eligible reviewed replacement is available.</strong> Keep training paused and do not continue this mission.</p>}</section>;
}
...
export function TrainingApp({signInLink,signOutLink}:{signInLink:ReactNode;signOutLink:ReactNode}){
 ...
 const [progression,setProgression]=useState<ProgressionProjection|null>(null);const [progressionLoading,setProgressionLoading]=useState(false);const [progressionError,setProgressionError]=useState('');
 ...
 const loadAdult=useCallback(async()=>{
  try{const result=await fetchTrainingProfiles();setProfiles(result.profiles);setActive(activeAdultProfileId(result.profiles));setMode(result.mode);setError('');if(result.mode==='signed-in'&&result.profiles.length){try{setMission(await fetchMissionProjection());}catch{setMission(null);}}else setMission(null);}
  catch(e){setError(e instanceof Error?e.message:'Cannot load training');setMode('error');}
 },[]);
 const loadProgress=useCallback(async()=>{setProgressionLoading(true);setProgressionError('');try{const result=await fetchProgressProjection();setProgression(result);return result;}catch(e){setProgressionError(e instanceof Error?e.message:'Your progress could not be loaded. Try again.');return null;}finally{setProgressionLoading(false);}},[]);
 const loadPlayer=useCallback(async()=>{try{const result=await fetchPlayerProjection();if(result){const currentMission=await fetchMissionProjection();setPlayer(result);setMission(currentMission);setProfiles([]);setMode('signed-in');setError('');void loadProgress();}return result;}catch(e){setError(e instanceof Error?e.message:'Cannot load your goalie');return null;}},[loadProgress]);
 ...
 const syncOffline=useCallback(async()=>{
  ...
  try{
   ...
   persistOfflineQueue(result.queue);if(result.mission){const confirmed={...result.mission,pendingSync:false};missionRef.current=confirmed;setMission(confirmed);if(['interrupted','completed'].includes(confirmed.status)){const refreshed=await fetchPlayerProjection();if(refreshed)setPlayer(refreshed);}if(confirmed.status==='completed')await loadProgress();}
   setOfflineStatus(result.status as OfflineStatus);
  }finally{syncRef.current=false;}
 },[persistOfflineQueue,loadProgress]);
 ...
 useEffect(()=>{let current=true;void loadInitialAccess(fetch,readTrainingResponse).then(async result=>{if(!current)return;if(result.kind==='player'){setPlayer(result.player as PlayerProjection);setMission(result.mission as MissionProjection);setProfiles([]);setMode('signed-in');setResumeFirstValue(true);setShowFirstValue(true);void loadProgress();}else if(result.kind==='adult'){const adultProfiles=result.profiles as Profile[];setProfiles(adultProfiles);setActive(activeAdultProfileId(adultProfiles));setMode('signed-in');if(adultProfiles.length){try{const adultMission=await fetchMissionProjection();if(current)setMission(adultMission);}catch{if(current)setMission(null);}}}else{setMode('guest');setProfiles([]);}setError('');}).catch(e=>{if(!current)return;setError(e instanceof Error?e.message:'Cannot load training');setMode('error');});return()=>{current=false;};},[loadProgress]);
 ...
 async function act(action:Action){
  ...
  try{
   ...
   if(playerMode){
    ...
    if(['safety-stop','complete-mission'].includes(missionAction)){const refreshed=await fetchPlayerProjection();if(refreshed)setPlayer(refreshed);if(missionAction==='complete-mission')await loadProgress();}
    return true;
   }
 ...
 async function goToAdult(){await loadAdult();setPlayer(null);setProgression(null);setProgressionError('');setShowFirstValue(false);navigate('Profile');}
 async function adoptReplacement(){
  if(!mission?.safetyHold?.replacement)return;busyRef.current=true;setBusy(true);setError('');
  try{const projected=await requestMissionMutation(mission,'replace-retired-activity',{activityKey:mission.safetyHold.activityKey});missionRef.current=projected;setMission(projected);toast.success('Reviewed replacement added');}
  catch(reason){setError(reason instanceof Error?reason.message:'The replacement could not be added. Reload and try again.');}
 ...
 return <main className="tf-app">
  ...
  <section id="training-main" className="tf-main">
   {tab==='Profile'&&!playerMode&&mission?.safetyHold&&profile?.role==='owner'&&<AdultSafetyReplacement hold={mission.safetyHold} busy={busy} onAdopt={()=>void adoptReplacement()}/>}
   ...
   {(tab==='Home'||tab==='Today')&&<>
    ...
    {state.safetyStopped?<div className="tf-error"><AlertTriangle/><h2>Training is paused.</h2><p>Tell a parent or guardian what happened. Rest does not erase your progress.</p><button className="tf-secondary" onClick={()=>void goToAdult()}>Adult review</button></div>:mission?.safetyHold?<MissionSafetyHold message={mission.safetyHold.message} replacement={mission.safetyHold.replacement} onAdult={()=>void goToAdult()}/>:mission?.status==='unavailable'?<MissionUnavailableState message={mission.unavailable?.message||'An adult needs to review your training plan.'} onAdult={()=>void goToAdult()}/>:missionAbandoned?<div className="tf-error"><AlertTriangle/><h2>This mission was ended.</h2><p>Your saved work remains in history. Return to an adult before starting another mission.</p><button className="tf-secondary" onClick={()=>void goToAdult()}>Return to adult</button></div>:<section className="tf-today"><div className="tf-today-top"><p className="tf-kicker">WEEK {session.week+1} · SESSION {session.day+1}</p><span><Clock size={16}/>{session.minutes} min planned</span></div><h2>{session.title}</h2><p className="tf-session-intro">{sessionDone?'Put the equipment away. Recovery is part of getting better.':`${session.blocks.length-completed} of ${session.blocks.length} activities left. One clear task at a time.`}</p><Progress value={completed/session.blocks.length*100} aria-label="Activities complete"/>
    ...
    {playerMode?(sessionDone&&mission?.completionSummary?<CompletionProgress summary={mission.completionSummary} pendingSync={mission.pendingSync===true}/>:<section className="tf-reward"><span className="tf-reward-icon"><Check size={32}/></span><div><p className="tf-kicker">PROGRESS SAVES AS YOU GO</p><h3>{sessionDone?'Mission saved':'Finish every activity with control'}</h3><p>{mission?.pendingSync?'Recent progress is stored on this device until it reconnects.':'Reloading or leaving will not erase confirmed sets.'}</p></div></section>):<section className="tf-reward"><span className="tf-reward-icon"><Trophy size={32}/></span><div><p className="tf-kicker">SESSION HISTORY</p><h3>{state.sessions.length?`${state.sessions.length} sessions completed`:'No completed sessions yet'}</h3><p>Player XP and rewards are confirmed only through the authoritative player progression system.</p></div></section>}
   ...
   {tab==='Progress'&&(playerMode?<ProgressionPanel projection={progression} loading={progressionLoading} error={progressionError} onRetry={()=>void loadProgress()}/>:<><p className="tf-kicker">OBSERVED ACCOMPLISHMENTS</p><h1>Evidence, not guesses.</h1><section className="tf-progress-summary"><div><strong>{state.sessions.length}</strong><span>Sessions finished</span></div><div><strong>{completed}/{session.blocks.length}</strong><span>Today’s activities</span></div><div><strong>{new Set(skillChecks.filter(c=>c.pathId===state.pathId&&c.passed&&c.skillId).map(c=>c.skillId)).size}/30</strong><span>Skills accomplished</span></div></section><h2>Earn the next path</h2><p>Show all 30 skills with control. Each development group must also be observed on two different days. A parent or coach records exactly what they saw.</p><div className="tf-skills">{GROUPS.map(group=>{const skills=SKILLS.filter(s=>s.group===group);const achieved=new Set(skillChecks.filter(c=>c.group===group&&c.pathId===state.pathId&&c.passed).map(c=>c.skillId));const days=new Set(skillChecks.filter(c=>c.group===group&&c.pathId===state.pathId&&c.passed).map(c=>c.date.slice(0,10))).size;return <article key={group}><div><h3>{group}</h3><span>{achieved.size}/{skills.length} skills</span></div><Progress value={achieved.size/skills.length*100} aria-label={`${group} skill accomplishments`}/><p>{Math.min(days,2)}/2 observed days · {RUBRICS[group]}</p></article>;})}</div></>)}
