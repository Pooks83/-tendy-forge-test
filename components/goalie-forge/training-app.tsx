...
type AccessShellProps={mode:'loading'|'signed-in'|'guest'|'error';error:string;signInLink:ReactNode;signOutLink:ReactNode;onPreview:()=>void;onReload:()=>void;onCreated:(id:string)=>Promise<void>};
export function AccessShell({mode,error,signInLink,signOutLink,onPreview,onReload,onCreated}:AccessShellProps) {
 return <main className="tf-app tf-access-app">
  ...
  <section className="tf-access-main" aria-labelledby="access-title">
   ...
   {mode==='signed-in'&&<EmptyAccount onCreated={onCreated} signOutLink={signOutLink}/>} 
  </section>
...
function ProfilePanel({profile,preview,profiles,active,onSwitch,state,busy,act,reload,onError,signOutLink}:{profile:Profile|undefined;preview:boolean;profiles:Profile[];active:string;onSwitch:(s:string)=>void;state:TrainingState;busy:boolean;act:(a:Action)=>Promise<boolean>;reload:()=>Promise<void>;onError:(s:string)=>void;signOutLink:ReactNode}) {
 ...
 return <><p className="tf-kicker">YOUR GOALIE PROFILE</p><h1>{profile?.nickname||'Preview goalie'}</h1><p>{profile?.team||'SCS Saints'} · {profile?.ageBand||'10–15'} · {PATHS.find(p=>p.id===state.pathId)?.name}</p><section className="tf-profile-card"><Shield/><h2>Private by default</h2><p>No public rankings, direct messages, or photos. Your progress is about your own practice.</p></section>
  {profiles.length>1&&<label className="tf-field">Training profile<select value={active||profiles[0]?.id} onChange={e=>onSwitch(e.target.value)}>{profiles.map(p=><option key={p.id} value={p.id}>{p.nickname} · {p.team}{p.role==='coach'?' (coach view)':''}</option>)}</select></label>}
  ...
  {showAdult&&<section className="tf-adult"><h2>{profile?.role==='coach'?'Coach observations':'Parent / guardian controls'}</h2><p>This signed-in adult account owns this workspace. Player view simplifies the screen; it is not a separate child login or a security lock.</p>
   ...
   <h3>Household</h3><button className="tf-secondary" onClick={()=>setAdding(!adding)}>Add another player</button>{adding&&<Setup onCreated={async id=>{await reload();onSwitch(id);setAdding(false);}}/>}
