...
type Step='loading'|'goalie-welcome'|'first-challenge'|'challenge-active'|'safety-stopped'|'first-win'|'first-today';
...
type Props={player:PlayerProjection;initialStep?:Step;initialMissionStarted?:boolean;resume?:boolean;onStartMission:(operationKey:string,alreadyStarted:boolean)=>Promise<void>;onStop:()=>Promise<boolean>;onParent:()=>Promise<void>};
...
async function todayViewed(operationKey:string){
 await fetch('/api/player-event',{method:'POST',headers:{'Content-Type':'application/json','Idempotency-Key':operationKey},body:JSON.stringify({eventName:'today_viewed'})});
}

export function PlayerFirstValueFlow({player,initialStep='goalie-welcome',initialMissionStarted=false,resume=false,onStartMission,onStop,onParent}:Props){
 const [state,dispatch]=useReducer(firstValueReducer,undefined,()=>({...initialFirstValueState(),step:resume?'loading':initialStep,missionStarted:initialMissionStarted}));
 const [loadAttempt,setLoadAttempt]=useState(0);
 useEffect(()=>{
  if(!resume)return;
  let current=true;
  void fetch('/api/first-challenge',{cache:'no-store'}).then(async response=>({ok:response.ok,data:await response.json() as {status?:string;remainingSeconds?:number;missionStatus?:string;error?:{message?:string}}})).then(({ok,data})=>{if(!current)return;if(ok)dispatch({type:'HYDRATE',status:data.status,remainingSeconds:data.remainingSeconds,missionStatus:data.missionStatus});else dispatch({type:'FAILED',message:data.error?.message||'Your challenge could not be loaded.'});}).catch(()=>{if(current)dispatch({type:'FAILED',message:'Your challenge could not be loaded. Try again.'});});
 ...
 useEffect(()=>{if(state.step==='first-today')void todayViewed(state.todayOperationKey).catch(()=>{});},[state.step,state.todayOperationKey]);
 ...
 async function stop(){
  dispatch({type:'BUSY'});
  try{if(await onStop())dispatch({type:'SAFETY_STOPPED'});else dispatch({type:'FAILED',message:'The safety stop was not confirmed. Try again.'});}catch(error){dispatch({type:'FAILED',message:error instanceof Error?error.message:'The safety stop was not confirmed. Try again.'});}
 }
 async function beginMission(){
  dispatch({type:'BUSY'});
  try{await onStartMission(state.missionOperationKey,state.missionStarted);}catch(error){dispatch({type:'FAILED',message:error instanceof Error?error.message:'Your mission could not be started. Try again.'});}
 }
 return <section className="tf-first-value" aria-live="polite">
  ...
  {state.step==='first-today'&&<><p className="tf-kicker">TODAY</p><h1>{state.missionStarted?'Your mission is ready to continue.':'Your first mission is ready.'}</h1><section className="tf-today"><h2>Build your base</h2><p>Start with clear, off-ice movement and tracking work. Rest and safety stops protect your progress.</p></section><button className="tf-primary" disabled={state.busy} onClick={()=>void beginMission()}>{state.busy?'OPENING…':state.missionStarted?'RESUME MISSION':'START MISSION'}</button></>}
