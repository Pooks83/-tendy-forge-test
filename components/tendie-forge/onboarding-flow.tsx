...
export function OnboardingFlow({initialStep='welcome',initialDraft,signOutLink,onHandoff,onCoach,legacyProfile}:Props){
 ...
 return <section className="tf-onboarding" aria-live="polite">
  ...
  {state.step==='gear'&&<><p className="tf-kicker">GEAR</p><h2>What is available at home?</h2><p>No equipment is required. Leave everything unselected for body-only missions.</p><div className="tf-choice-grid">{equipment.map(([id,label])=><label key={id} className="tf-choice"><input type="checkbox" checked={state.data.equipment.includes(id)} onChange={()=>toggleEquipment(id)}/><span>{label}</span></label>)}</div>{draftError&&<p className="tf-error" role="alert">{draftError}</p>}<button className="tf-primary" disabled={savingDraft} onClick={()=>void advance('training-plan')}>{savingDraft?'SAVING…':'NEXT'}</button>{accountActions}</>}
