...
type ActivityRow={id:string;mission_instance_id:string;activity_key:string;ordinal:number;status:string;result_json:string|null;rest_remaining_seconds:number;started_at:string|null;completed_at:string|null;updated_at:string};
...
type ExecutionActivity={key:string;ordinal:number;status:string;result:MissionResult|null;restRemainingSeconds:number;startedAt?:string;completedAt?:string;updatedAt?:string};
type MissionExecution={missionId:string;status:string;revision:number;currentActivityIndex:number;contentVersion:string;activities:ExecutionActivity[];startedAt:string;updatedAt?:string;completedAt?:string;pausedAt?:string;interruptedAt?:string;abandonedAt?:string};
...
async function activityRows(db:D1Database,missionInstanceId:string){
 return (await db.prepare('SELECT id,mission_instance_id,activity_key,ordinal,status,result_json,rest_remaining_seconds,started_at,completed_at,updated_at FROM activity_instances WHERE mission_instance_id=? ORDER BY ordinal').bind(missionInstanceId).all<ActivityRow>()).results;
}

function projection(row:MissionRow,activities:ActivityRow[]){
 ...
 return {
  ...
  activities:activities.map(item=>({key:item.activity_key,ordinal:item.ordinal,status:externalStatus(item.status),result:parseJson(item.result_json,null),restRemainingSeconds:item.rest_remaining_seconds,requiredSets:snapshot.blocks?.find(block=>block.id===item.activity_key)?.sets??0,...(item.started_at?{startedAt:item.started_at}:{}),...(item.completed_at?{completedAt:item.completed_at}:{})})),
...
function executionFromRows(row:MissionRow,activities:ActivityRow[]):MissionExecution{
 return {
  ...
  activities:activities.map(item=>({key:item.activity_key,ordinal:item.ordinal,status:internalStatus(item.status),result:parseJson(item.result_json,null),restRemainingSeconds:item.rest_remaining_seconds,...(item.started_at?{startedAt:item.started_at}:{}),...(item.completed_at?{completedAt:item.completed_at}:{}),updatedAt:item.updated_at})),
...
async function hydrateLegacyMission(db:D1Database,identity:AdultIdentity,row:MissionRow,session:Session){
 ...
 const migrated=session.blocks.map((block,ordinal)=>{
  ...
  return {id:crypto.randomUUID(),key:block.id,ordinal,status:complete?'COMPLETED':remaining>0?'RESTING':active?'IN_PROGRESS':'READY',result,remaining,startedAt:active||complete?row.started_at:null,completedAt:complete?now:null};
 });
 ...
 await db.batch([
  ...
  ...migrated.map(activity=>db.prepare('INSERT OR IGNORE INTO activity_instances(id,mission_instance_id,activity_key,ordinal,status,result_json,rest_remaining_seconds,started_at,completed_at,updated_at) SELECT ?,?,?,?,?,?,?,?,?,? WHERE EXISTS(SELECT 1 FROM mission_instances WHERE id=? AND content_version=?)').bind(activity.id,row.id,activity.key,activity.ordinal,activity.status,activity.result?JSON.stringify(activity.result):null,activity.remaining,activity.startedAt,activity.completedAt,now,row.id,MISSION_CONTENT_VERSION)),
...
export async function startCurrentMission(db:D1Database,identity:AdultIdentity,operationKey:string,now=new Date()){
 ...
 const rows:ActivityRow[]=execution.activities.map((activity:{key:string;ordinal:number;status:string})=>({id:crypto.randomUUID(),mission_instance_id:id,activity_key:activity.key,ordinal:activity.ordinal,status:activity.status,result_json:null,rest_remaining_seconds:0,started_at:null,completed_at:null,updated_at:timestamp}));
 ...
 try{
  await db.batch([
   ...
   ...rows.map(item=>db.prepare('INSERT INTO activity_instances(id,mission_instance_id,activity_key,ordinal,status,result_json,rest_remaining_seconds,started_at,completed_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(item.id,id,item.activity_key,item.ordinal,item.status,null,0,null,null,timestamp)),
...
function legacyProjection(raw:string,snapshot:Session,next:MissionExecution,timestamp:string){
 ...
 return {...state,sets,rests,answers,sessions};
}
...
export async function mutateCurrentMission(db:D1Database,identity:AdultIdentity,input:MutationInput,operationKey:string,now=new Date()){
 ...
 if(active.player.training.safetyStopped)throw canonicalError('SAFETY_STOPPED','Training is paused. Ask a parent or guardian to check in before continuing.',409);
 const before=executionFromRows(loaded.row,loaded.activities);const snapshot=parseJson<ExecutionSnapshot>(loaded.row.execution_snapshot_json,snapshotFor(active.session));
 ...
 const timestamp=now.toISOString();const next=transitionMission(before,transitionAction(input),timestamp) as MissionExecution;
 ...
 const activityStatements=nextRows.map((activity,index)=>db.prepare(`UPDATE activity_instances SET status=?,result_json=?,rest_remaining_seconds=?,started_at=?,completed_at=?,updated_at=? WHERE id=? AND ${condition}`).bind(activity.status,activity.result?JSON.stringify(activity.result):null,activity.restRemainingSeconds||0,activity.startedAt||null,activity.completedAt||null,timestamp,loaded.activities[index].id,loaded.row.id,next.revision,active.profileId,profile.revision+1));
 try{
