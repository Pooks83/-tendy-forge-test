...
type PlayerRow={id:string;nickname:string;age_band:string;catches:string|null;experience:string|null;equipment_json:string|null;planned_days_json:string|null;mission_minutes:number|null;setup_status:string;state:string;revision:number};
...
export async function reconcileLegacySetup(db:D1Database,identity:AdultIdentity,profileId:string,rawInput:unknown,operationKey:string){
 ...
 try{
  await db.batch([
   ...
   db.prepare('INSERT INTO consent_records(id,account_id,profile_id,consent_version,purposes_json,policy_version,accepted_at) VALUES(?,?,?,?,?,?,?)').bind(crypto.randomUUID(),identity.id,profileId,input.consentVersion,JSON.stringify(['training','progress','safety']),input.policyVersion,now),
   ...
   db.prepare('INSERT INTO active_player_context(account_id,profile_id,updated_at) VALUES(?,?,?) ON CONFLICT(account_id) DO UPDATE SET profile_id=excluded.profile_id,updated_at=excluded.updated_at').bind(identity.id,profileId,now),
   db.prepare('INSERT INTO audit_events(id,actor_account_id,profile_id,event_type,metadata_json,created_at) VALUES(?,?,?,?,?,?)').bind(crypto.randomUUID(),identity.id,profileId,'LEGACY_SETUP_RECONCILED',JSON.stringify({consentVersion:input.consentVersion,policyVersion:input.policyVersion}),now),
...
async function playerRow(db:D1Database,profileId:string){
 const row=await db.prepare('SELECT id,nickname,age_band,catches,experience,equipment_json,planned_days_json,mission_minutes,setup_status,state,revision FROM training_profiles WHERE id=?').bind(profileId).first<PlayerRow>();
...
function project(row:PlayerRow){
 ...
 return buildPlayerProjection({id:row.id,nickname:row.nickname,ageBand:row.age_band,catches:row.catches,experience:row.experience,equipment:parseArray(row.equipment_json),plannedDays:parseArray(row.planned_days_json),missionMinutes:row.mission_minutes,setupStatus:row.setup_status,revision:row.revision},JSON.parse(row.state));
}
