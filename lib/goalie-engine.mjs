const initialState = {
  organization: {
    id: "scs-saints",
    name: "St. Clair Shores Saints",
    shortName: "SCS Saints",
    developmentLabel: "Organization-wide goalie development",
    activeTeamId: "2017-aa",
    teams: [
      { id: "2017-aa", label: "2017 AA", division: "10U AA", goalieCount: 3 },
      { id: "2016-aa", label: "2016 AA", division: "10U AA", goalieCount: 2 },
      { id: "2015-aa", label: "2015 AA", division: "12U AA", goalieCount: 2 },
    ],
  },
  player: {
    nickname: "Henry",
  ...
  roster: [
    {
      ...
      teamId: "2017-aa",
      signal: "Check in",
      detail: "Re-test for explosiveness is due",
      tone: "attention",
      focus: "Tracking through traffic",
    },
    {
      ...
      teamId: "2017-aa",
      signal: "On rhythm",
      detail: "Completed this week’s focus",
      tone: "building",
      focus: "Balance in the set position",
    },
    {
      id: "morgan",
      nickname: "Morgan",
      teamId: "2017-aa",
      signal: "Rest day",
      detail: "Parent-selected recovery day",
      tone: "holding",
      focus: "Recovery protected",
    },
    {
      id: "mia",
      nickname: "Mia",
    ...
    },
    {
      ...
      teamId: "2016-aa",
      signal: "Check in",
      detail: "Next reset session is due",
      tone: "attention",
      focus: "Calm reset after a goal",
    },
    {
      id: "olivia",
      nickname: "Olivia",
      teamId: "2015-aa",
      signal: "On rhythm",
      detail: "Tracking focus completed",
      tone: "building",
      focus: "Tracking through traffic",
    },
    {
...
export function createInitialState(overrides = {}) {
  ...
  return {
    ...next,
    ...overrides,
    organization: { ...next.organization, ...(overrides.organization ?? {}) },
...
export function selectOrganizationTeam(state, teamId) {
  const next = clone(state);
  const isOrganizationView = teamId === "all";
  const isKnownTeam = next.organization.teams.some((team) => team.id === teamId);
  if (!isOrganizationView && !isKnownTeam) return next;
  next.organization.activeTeamId = teamId;
  return next;
}

export function getSelectedTeam(state) {
  if (state.organization.activeTeamId === "all") return null;
  return state.organization.teams.find((team) => team.id === state.organization.activeTeamId) ?? null;
}

export function getSelectedTeamRoster(state) {
  const activeTeamId = state.organization.activeTeamId;
  if (activeTeamId === "all") return state.roster;
  return state.roster.filter((goalie) => goalie.teamId === activeTeamId);
}
...
export function completeMission(state) {
  ...
  if (next.player.ownedRewardIds.includes(rewardId)) {
    ...
    next.reward = {
      ...
      type: "cosmetic",
      amount: 0,
      label: "Saints Redline Mask Finish",
    };
  }
...
export function setCoachFocus(state, focus) {
  const next = clone(state);
  if (!next.coach.linked) return next;
  next.coach.focus = focus;
  next.mission.nextBoundaryFocus = focus;
  return next;
}

export function setCoachLink(state, linked) {
  const next = clone(state);
  next.coach.linked = linked;
  if (!linked) {
    next.coach.focus = null;
    next.coach.presetMessage = null;
    next.mission.nextBoundaryFocus = null;
  } else if (!next.coach.focus) {
    next.coach.focus = "Tracking through traffic";
    next.coach.presetMessage = "Keep finding the puck first. Your next save starts before the shot.";
  }
  return next;
}

export function updateParentPreferences(state, changes) {
  const next = clone(state);
  next.parent = { ...next.parent, ...changes };
  return next;
}

export function acknowledgeSafetyStop(state, decision) {
  const next = clone(state);
  if (!next.parentActionRequired) return next;
  next.parentActionRequired = false;
  next.safety = {
    ...next.safety,
    active: false,
    parentDecision: decision,
  };
  next.parent.lastSafetyDecision = decision;
  return next;
}
