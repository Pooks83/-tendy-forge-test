...
const REASONS=new Set(['safe-substitution','app-closed','adult-ended']);
...
function validItem(item){
 ...
 if(body.reason!==undefined&&(typeof body.reason!=='string'||!REASONS.has(body.reason)))return false;
...
export function hasPendingSafetyStop(queue,profileContextId){return queue.some(item=>item.profileContextId===profileContextId&&item.body.action==='safety-stop');}
...
export async function reconcileOfflineQueue({queue,profileContextId,send,fetchAuthoritative}){
 ...
 while(true){
  ...
  try{latest=await send(item);remaining=remaining.filter(candidate=>candidate.operationKey!==item.operationKey);continue;}catch(error){
   ...
   remaining=remaining.map(candidate=>candidate.operationKey===item.operationKey?rebased:candidate);
