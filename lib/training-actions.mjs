import {PATHS,SKILLS,EVALUATION_SECTIONS,WEEKS,buildSession,recordSet,finishSession,nextSession,canAdvance} from './training.mjs';
export function applyAction(state,action,actor,date=new Date().toISOString()) {
 ...
 switch(action.type) {
  ...
  case 'evaluate': {
   ...
   if(!Array.isArray(action.ratings)||action.ratings.length!==EVALUATION_SECTIONS.length||!action.ratings.every(n=>Number.isInteger(n)&&n>=0&&n<=3)||!['none','positioning','tracking','movement','save selection','execution','decision-making'].includes(action.cause)||typeof action.note!=='string'||action.note.trim().length<8||action.note.length>600) throw new Error('Invalid evaluation: complete all ten sections and add a specific observation');
  ...
  case 'check': {
   ...
   if(!skill) throw new Error('Choose a valid skill before recording evidence');
   if(typeof action.passed!=='boolean'||typeof action.note!=='string'||action.note.trim().length<8||action.note.length>600) throw new Error('Add a specific observed example (8–600 characters)');
   const check={skillId:skill.id,group:skill.group,pathId:state.pathId,passed:action.passed,note:action.note.trim(),reviewedBy:actor.id,date};
