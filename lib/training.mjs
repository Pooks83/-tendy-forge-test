...
// The same 30 skills stay visible at every path. The proof becomes more demanding,
// so advancement reflects observed accomplishments rather than elapsed time.
const skillRows=[
 ['Ready position','Move','Finish in a balanced ready stance.'],
 ['Balance','Move','Hold steady without the knee falling inward.'],
 ['Controlled footwork','Move','Move quietly and stop under control.'],
 ['Lateral power','Move','Push sideways and land balanced.'],
 ['Jumping and landing','Move','Take off strongly and land quietly.'],
 ['Short acceleration','Move','Start quickly and slow down safely.'],
 ['Bodyweight strength','Move','Keep good body position through every rep.'],
 ['Core control','Recover','Keep the hips and trunk steady while moving.'],
 ['Comfortable mobility','Recover','Use a pain-free range without forcing it.'],
 ['Hand-eye coordination','See','Watch the soft ball all the way to the hands.'],
 ['Ball tracking','See','Keep the eyes and head with the full ball path.'],
 ['Target control','React','Look first, then send the soft ball to the target.'],
 ['Positioning concepts','Think','Explain where the goalie should be and why.'],
 ['Safe stick-and-ball control','React','Control a soft ball with the stick below the knees.'],
 ['Communication','Compete','Use one short, clear call before the action.'],
 ['Reset routine','Recover','Breathe, look, and return to ready after a miss.'],
 ['Pre-shot information gathering','See','Scan early, then return the eyes before the action.'],
 ['Release reading and deception','React','React to the real release cue, not the first fake.'],
 ['Pattern recognition','Think','Name the most likely next play without guessing.'],
 ['Save selection and decisions','Think','Choose the simplest useful response for the situation.'],
 ['Transition and recovery','Recover','Find the ball first, move toward it, then reset.'],
 ['Broken plays and chaos','React','Update the read when the expected action changes.'],
 ['Behind-net and low-zone reading','Think','Track the carrier and name the front-side threat.'],
 ['Traffic and sightlines','See','Make a small head move to find a clear viewing window.'],
 ['Preventing chances','Think','Choose the action that safely prevents the next chance.'],
 ['Game situations','Think','Explain how the situation changes the safest decision.'],
 ['Film intelligence','Think','Use repeated clues without treating one clip as certainty.'],
 ['Quality under fatigue','Compete','Keep the same control late or choose rest.'],
 ['Adaptability and learning','Compete','Apply one coaching cue and describe what changed.'],
 ['Competitive behavior','Compete','Own the miss, reset, and give the next rep full focus.'],
];
export const SKILLS=skillRows.map(([name,group,cue],i)=>({
 id:i+1,name,group,cue,
 benchmarks:{
  foundation:`Explain or show with control: ${cue}`,
  builder:`Repeat twice with the same control: ${cue}`,
  performance:`Apply after the cue or situation changes: ${cue}`,
 },
}));

export const EVALUATION_SECTIONS=[
 {id:'warmup',name:'Warm-up and ready stance',task:'Use marching, side steps, and a balanced ready position for 5 minutes.',watchFor:['Comfortable stance','Stable head and hands','Balanced stops'],context:'off-ice'},
 {id:'footwork',name:'Footwork and balance',task:'Complete controlled steps between two floor markers for 5 minutes.',watchFor:['Quiet feet','Efficient direction changes','No knee collapse'],context:'off-ice'},
 {id:'tracking',name:'Tracking',task:'Complete 10 gentle two-hand wall-ball attempts.',watchFor:['Eyes follow the full path','Head arrives before the hands','Misses are reacquired'],context:'off-ice'},
 {id:'hands',name:'Hand-eye response',task:'Complete 10 alternating-hand wall-ball attempts or the easier two-hand version.',watchFor:['Soft catches','Hands stay in view','Control before speed'],context:'off-ice'},
 {id:'lateral',name:'Lateral power',task:'Complete 3 small side jumps or steps each way with a 2-second hold.',watchFor:['Immediate push','Quiet landing','Balanced finish'],context:'off-ice'},
 {id:'reading',name:'Release reading and decisions',task:'Complete five seated screen scenarios with one changed cue in each.',watchFor:['Waits for useful information','Explains the choice','Does not react to every fake'],context:'off-ice'},
 {id:'recovery',name:'Find, move, reset',task:'Complete 6 gentle ball rolls to alternating sides.',watchFor:['Eyes find the ball first','Moves toward the ball','Returns ready'],context:'off-ice'},
 {id:'sightlines',name:'Scanning and sightlines',task:'Complete 6 look-and-find reps around a stable chair with no thrown ball.',watchFor:['Small head adjustment','Early safe scan','Eyes return to the target'],context:'off-ice'},
 {id:'control',name:'Ball control and communication',task:'Complete 8 soft-ball target passes and call the target before each pass.',watchFor:['Looks before passing','Accurate gentle control','Clear communication'],context:'off-ice'},
 {id:'durability',name:'Quality, reset, and coachability',task:'Repeat one earlier easy task after rest and apply one coaching cue.',watchFor:['Technique stays controlled','Calm response after a miss','Observable use of feedback'],context:'off-ice'},
];
export const DRILLS = {
...
/** @typedef {{version:number,cycle?:number,pathId:string,week:number,day:number,sets:Record<string,boolean>,rests:Record<string,{until:number,remaining:number}>,sessions:Array<{id:string,date:string,pathId:string,week:number,day:number}>,checks:Array<{skillId?:number,group:string,pathId:string,passed:boolean,note:string,reviewedBy:string,date:string}>,evaluations:Array<{ratings:number[],cause:string,note:string,reviewedBy:string,date:string,pathId:string,context:string}>,answers:Record<string,{choice:number,correct:boolean}>,safetyStopped:boolean}} TrainingState */
/** @returns {TrainingState} */
export const newTrainingState=()=>({version:2,pathId:'foundation',week:0,day:0,sets:{},rests:{},sessions:[],checks:[],evaluations:[],answers:{},safetyStopped:false});
export function normalizeTrainingState(value) {
 const base=newTrainingState();
 if(!value||typeof value!=='object')return base;
 const path=PATHS.find(candidate=>candidate.id===value.pathId)||PATHS[0];
 const week=Number.isInteger(value.week)&&value.week>=0&&value.week<20?value.week:0;
 const day=Number.isInteger(value.day)&&value.day>=0&&value.day<path.days?value.day:0;
 return {...base,version:2,pathId:path.id,week,day,cycle:Number.isInteger(value.cycle)&&value.cycle>=0?value.cycle:0,sets:value.sets&&typeof value.sets==='object'?value.sets:{},rests:value.rests&&typeof value.rests==='object'?value.rests:{},sessions:Array.isArray(value.sessions)?value.sessions:[],checks:Array.isArray(value.checks)?value.checks:[],evaluations:Array.isArray(value.evaluations)?value.evaluations:[],answers:value.answers&&typeof value.answers==='object'?value.answers:{},safetyStopped:value.safetyStopped===true};
}
...
export function canAdvance(state) {
 const valid=(state.checks||[]).filter(c=>c.pathId===state.pathId&&c.passed===true&&c.reviewedBy&&Number.isInteger(c.skillId));
 const everySkill=SKILLS.every(skill=>valid.some(check=>check.skillId===skill.id));
 const twoDaysPerGroup=GROUPS.every(group=>new Set(valid.filter(c=>c.group===group).map(c=>c.date.slice(0,10))).size>=2);
 return everySkill&&twoDaysPerGroup;
}
