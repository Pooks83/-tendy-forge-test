...
function failure(code,message){
 ...
 error.code=code;
 error.status=code==='SESSION_ALREADY_COMPLETED'?409:400;
 return error;
}
...
export function createMissionExecution({missionId,activityKeys,contentVersion}){
 ...
 return {
  ...
  activities:activityKeys.map((key,ordinal)=>({key,ordinal,status:'READY',result:null})),
 };
}
...
export function transitionMission(state,action,at=new Date().toISOString()){
 ...
 switch(action.type){
  ...
  case 'END_REST': {
   ...
   return updateCurrent(state,activity,{status:'IN_PROGRESS',restRemainingSeconds:0},at);
  }
  case 'COMPLETE_ACTIVITY': {
   ...
   return revise(state,{activities,currentActivityIndex:Math.min(activity.ordinal+1,activities.length)},at);
  }
  case 'SKIP_ACTIVITY': {
