import {CONSENT_VERSION,POLICY_VERSION} from './identity-contract.mjs';

export const ONBOARDING_STEPS=['welcome','adult-account','parent-permission','create-goalie','gear','training-plan','handoff'];

const blankData=()=>({nickname:'',ageBand:'',catches:'left',experience:'new',equipment:[],spaces:[],plannedDays:[],missionMinutes:25,consentAccepted:false,analyticsAllowed:false});
...
export function restoreOnboardingState(draft){
 if(!draft||typeof draft!=='object'||!RESUMABLE_STEPS.has(draft.step)||typeof draft.operationKey!=='string'||draft.operationKey.length<8||draft.consentAccepted!==true||draft.consentVersion!==CONSENT_VERSION||draft.policyVersion!==POLICY_VERSION||!draft.data||typeof draft.data!=='object')return initialOnboardingState();
 ...
 return {
  ...
  data:{
   ...
   spaces:Array.isArray(draft.data.spaces)?draft.data.spaces:[],
